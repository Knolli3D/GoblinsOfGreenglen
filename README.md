...
- **`CampaignCatalog.gd`** is the source of truth for stable region/level IDs, scene paths, prerequisites, route types, and map metadata. Region 1 maps to the current six playable scenes; Region 2 is deliberately unreleased scaffolding for eight main locations and two optional bonus branches; Regions 3–5 extend the roadmap as unreleased serpentine placeholders (10/12/14 main locations, empty scene paths) chained sequentially behind it.
